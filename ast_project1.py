...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec  6 19:28:46 2018
...
tr = []
PRECISION = 0.03
for y in range(len(a0)):
    t_inspiral = numpy.zeros(runs)
    nsa0 = 0
    nproblem = 0
    for i in range(runs):
        a = a0[y]
        ...
        #while s > s_crit and J >= J_lc:
            ...
            t_inspiral[i] += P
            if t_inspiral[i] > 14.7481*3.1415e16:
                nproblem += 1
                break
            ...
            if (J < 0.9*Jmax):
                ...
                deltaJ = -nstep*delta1Jsc + x()*math.sqrt(nstep)*delta2Jsc - nstep*deltaJGW
            else:
                nstep = max(1,min(dJ,dE))
                deltaJ = -nstep*deltaJGW-nstep*delta1Jsc
        ...
        if (P <= 10000):
            ...
            DFJa0J.append(0)
    Sa0.append(nsa0)
    problem.append(nproblem)
f = open( 'file100.py', 'a' )
f.write( 'time:' + repr(datetime.datetime.now()) + '\n' 'Sa0 = ' + repr(Sa0) + '\n' 'problem = ' + repr(problem) + '\n' 'DFJa0J = ' + repr(DFJa0J) + '\n' 'ecc = ' + repr(ecc) + '\n' 'a_final = ' + repr(a_final) + '\n' )
f.close()